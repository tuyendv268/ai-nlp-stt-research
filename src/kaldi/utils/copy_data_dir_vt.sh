...
cat $srcdir/wav.scp | awk -v p=$utt_suffix '{printf("%s %s%s\n", $1, $1, p);}' > $destdir/reco_map
...
if [ -f $srcdir/segments ]; then
  ...
  utils/apply_map.pl -f 2 $destdir/reco_map <$destdir/segments >$destdir/segments1
  mv $destdir/segments1 $destdir/segments
  utils/apply_map.pl -f 1 $destdir/reco_map <$srcdir/wav.scp >$destdir/wav.scp
