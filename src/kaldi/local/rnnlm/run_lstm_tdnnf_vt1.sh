...
dir=expTDNN2/rnnlm_lstm_tdnnf_vt1
embedding_dim=500
lstm_rpd=200
lstm_nrpd=200
...
ac_model_dir=expTDNN2/chain/cnn_tdnn_swb_1a_online
ngram_order=4
...
wordlist=data/lang_nosp_pocolm/words.txt
text_dir=tmp-preparing/rnnlm/text-dir
mkdir -p $dir/config
set -e

for f in $wordlist; do
  [ ! -f $f ] && \
...
#wsj   1   1.0
#EOF
  cp tmp-preparing/rnnlm/config-dir/data_weights.txt $dir/config/data_weights.txt
  ...
  rnnlm/choose_features.py --unigram-probs=$dir/config/unigram_probs.txt \
                           ...
                           --min-frequency 1.0e-05 \
                           --special-words='<s>,</s>,<brk>,<unk>,<noise>,ah,wow,woa,uhm' \
...
if [ $stage -le 2 ]; then
  ...
  rnnlm/prepare_rnnlm_dir.sh --unigram-factor 500.0 \
