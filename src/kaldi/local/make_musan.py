  ...
  for utt in utt2vocals:
    if utt in utt2wav:
      if use_vocals or not utt2vocals[utt]:
        ...
        utt2wav_str = utt2wav_str + utt + " sox " + utt2wav[utt] + " -r 16k -t wav -c 1 -b 16 - |\n"
      num_good_files += 1
    else:
...
def prepare_music2(root_dir):
  utt2spk = {}
  utt2wav = {}
  num_good_files = 0
  num_bad_files = 0
  speech_dir = os.path.join(root_dir, "music")
...
def prepare_vietnamese(root_dir):
  utt2spk = {}
  utt2wav = {}
  num_good_files = 0
  num_bad_files = 0
  noise_dir = os.path.join(root_dir, "vietnamese")
  for root, dirs, files in os.walk(noise_dir):
    for file in files:
      file_path = os.path.join(root, file)
      if file.endswith(".wav"):
...
def main():
  ...
  #tt2spk_music, utt2wav_music = prepare_music2(in_dir, use_vocals)
  utt2spk_music, utt2wav_music = prepare_music2(in_dir)
  utt2spk_speech, utt2wav_speech = prepare_speech(in_dir)
  utt2spk_noise, utt2wav_noise = prepare_noise(in_dir)
  utt2spk_vietnamese, utt2wav_vietnamese = prepare_vietnamese(in_dir)
  utt2spk = utt2spk_speech + utt2spk_music + utt2spk_noise + utt2spk_vietnamese
  utt2wav = utt2wav_speech + utt2wav_music + utt2wav_noise + utt2wav_vietnamese
