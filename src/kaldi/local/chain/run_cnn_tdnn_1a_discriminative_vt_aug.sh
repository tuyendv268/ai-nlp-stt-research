...
srcdir=expTDNN2/chain/cnn_tdnn_swb_1a_online
train_data_dir=data/mfcc_hires/All-140h_cleaned_3speed_aug
online_ivector_dir=expTDNN2/nnet3/ivectors_All-140h_cleaned_3speed_aug_max3
...
if [ $frame_subsampling_factor -ne 1 ]; then
  if [ $stage -le 0 ]; then
    ...
    for x in `seq -$[frame_subsampling_factor/2] $[frame_subsampling_factor/2]`; do
      steps/shift_feats.sh --cmd "$train_cmd --max-jobs-run 8" --nj 8 \
...
if [ $stage -le 1 ]; then
  ...
  nj=8 # have a high number of jobs because this could take a while, and we might
...
if [ -z "$degs_dir" ]; then
  ...
  if [ $stage -le 3 ]; then
    ...
    steps/nnet3/get_egs_discriminative.sh \
      --cmd "$decode_cmd --max-jobs-run $max_jobs --mem 40G" --stage $get_egs_stage --cmvn-opts "$cmvn_opts" \
