...
srcdir=expTDNN2/chain/tdnn_7p
train_data_dir=data/mfcc_hires/Kaldi-data-FPT-data-fext-nonoise-20180805_3wavspeed_Volummed
online_ivector_dir=expTDNN2/nnet3_cleaned/ivectors_data/mfcc_hires/Kaldi-data-FPT-data-fext-nonoise-20180805_3wavspeed_Volummed_max20
...
if [ $stage -le 1 ]; then
  ...
  nj=4 # have a high number of jobs because this could take a while, and we might
         # have some stragglers.
  steps/nnet3/align.sh  --cmd "run.pl" --use-gpu "true" \
    --config "conf/sMBR.conf" \
...
if [ -z "$lats_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    subsplit=10 # number of jobs that run per job (but 2 run at a time, so total jobs is 80, giving
