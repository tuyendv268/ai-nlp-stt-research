...
if [ $frame_subsampling_factor -ne 1 ]; then
  if [ $stage -le 0 ]; then
    ...
    for x in `seq -$[frame_subsampling_factor/2] $[frame_subsampling_factor/2]`; do
      steps/shift_feats.sh --cmd "$train_cmd --max-jobs-run 16" --nj 50 \
...
if [ $stage -le 1 ]; then
  ...
  nj=16 # have a high number of jobs because this could take a while, and we might
...
if [ -z "$lats_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    steps/nnet3/make_denlats_gpu.sh --cmd "$decode_cmd" \
