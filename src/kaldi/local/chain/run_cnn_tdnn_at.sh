...
dir=expTDNN2/chain/cnn_tdnn_at_last
...
if [ $stage -le 12 ]; then
  ...
  prefinal-layer name=prefinal-chain input=attention1 $prefinal_opts small-dim=256 big-dim=1536
  output-layer name=output include-log-softmax=false dim=$num_targets $output_opts
  # adding the layers for xent branch
  prefinal-layer name=prefinal-xent input=attention1 $prefinal_opts small-dim=256 big-dim=1536
...
if [ $stage -le 13 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    --trainer.input-model "expTDNN2/chain/cnn_tdnn_at_last/564.mdl" \
    --cmd "$train_cmd" \
    ...
    --trainer.optimization.initial-effective-lrate 0.000025 \
    --trainer.optimization.final-effective-lrate 0.0000025 \
