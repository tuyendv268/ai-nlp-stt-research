...
do_combine() {
  ...
  ######################################################
  #Thinh change here. Need to check permission of $temp_dir
  njob=$(expr $nj - 1)

  for ((i=$njob; i >= 0 ; i--))
  do
    echo "utils/split_scp.pl --utt2spk=$data/utt2spk -j $nj $i $temp_dir/$ark.scp $temp_dir/$ark.temp.$(expr $i + 1).scp || exit 1"
    # echo "$i more file left."
  done | xargs -I {} -P $njob sh -c "{}"


  $cmd JOB=1:$nj $dest/log/chop_combined_$entities.JOB.log \
    $copy_program scp:$temp_dir/$ark.temp.JOB.scp  "ark:| gzip -c > $dest/$ark.JOB.gz" || exit 1
  #Thinh: Done change
  ######################################################
