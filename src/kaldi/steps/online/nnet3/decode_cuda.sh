...
if [ "$online_config" == "" ]; then
  online_config=$srcdir/conf/online_cuda.conf;
fi
...
if [ -f $srcdir/frame_subsampling_factor ]; then
  ...
  frame_subsampling_opt="--frame-subsampling-factor=$(cat $srcdir/frame_subsampling_factor)"
      ...
      # --cuda-cache-memory=false \
      # --cuda-worker-threads=2 \
      # --gpu-feature-extract=false \
      # --optimization.move-sizing-commands=false \
...
if [ $stage -le 0 ]; then
  $cmd JOB=1:$nj $dir/log/decode.JOB.log \
    batched-wav-nnet3-cuda \
      ...
      $frame_subsampling_opt --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam --acoustic-scale=$acwt \
