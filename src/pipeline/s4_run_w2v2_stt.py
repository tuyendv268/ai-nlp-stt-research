...
def main(metadata, pred_metadata_path, batch_size=8):
    ...
    with open(pred_metadata_path, "a+") as f:
        # metadata = sorted(metadata, key=lambda x: x["end_time"] - x["start_time"])
        ...
        for index in tqdm(
            ...
            batch = batches[index]
            batch = list(map(json.loads, batch))
            
            batch = [sample for sample in batch if (sample["end_time"]- sample["start_time"] > 0.4)]
            ...
            def prepare_sample(sample):
                ...
                audio_segment = audio[0][int(start_time*sr):int(end_time*sr)]
                
                return audio_segment

            #####
            audio_batch = list(map(prepare_sample, batch))
            if len(audio_batch) != len(batch):
                continue
            ...
            for i in range(len(batch)):
                ...
                audio_filepath = sample["audio_filepath"]
                inter_text = sample["matching_segments"]
                start_time = sample["start_time"]
                end_time = sample["end_time"]
...
if __name__ == "__main__":        
    filepaths = [
        "/data/asr/kaldi/data/f88_2k4h/segments.jsonl",
    ]
    pred_metadata_dir = "/data/asr-research/data/stt_metadata_w2v2_part_3"
    
    if not os.path.exists(pred_metadata_dir):
        os.mkdir(pred_metadata_dir)

    metadata_dir = "/data/asr-research/data/stt_metadata_w2v2_part_3"
    ...
    for metadata_path in filepaths:
        ...
        # load metadata
        metadata = load_jsonl_data(metadata_path, load_json_obj=False)
