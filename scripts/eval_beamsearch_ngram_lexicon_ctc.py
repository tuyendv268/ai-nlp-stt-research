...
def main(cfg: EvalBeamSearchNGramConfig):
    ...
    elif cfg.nemo_model_file.endswith('.ckpt'):
        from nemo.collections.asr.models.ctc_bpe_models import EncDecCTCModelBPE
        asr_model = EncDecCTCModelBPE.load_from_checkpoint(cfg.nemo_model_file, map_location=torch.device(cfg.device))
    else:
        logging.warning(
    ...
    if cfg.probs_cache_file and os.path.exists(cfg.probs_cache_file):
        ...
        with autocast():
            with torch.no_grad():
                ...
                all_logits = asr_model.transcribe(audio_file_paths, batch_size=cfg.acoustic_batch_size, return_hypotheses=True)
                all_logits = [all_logit.y_sequence for all_logit in all_logits]
    ...
    for batch_idx, probs in enumerate(all_probs):
        ...
        if isinstance(asr_model, EncDecHybridRNNTCTCModel):
            pred_text = asr_model.ctc_decoding.ctc_decoder_predictions_tensor(preds_tensor)
            pred_text = pred_text[0].text
        else:
            # pred_text = asr_model._wer.decoding.ctc_decoder_predictions_tensor(preds_tensor)
            pred_text = asr_model.wer.decoding.ctc_decoder_predictions_tensor(preds_tensor)
            pred_text = pred_text[0].text
